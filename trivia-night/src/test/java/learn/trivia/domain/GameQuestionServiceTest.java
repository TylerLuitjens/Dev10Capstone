...
public class GameQuestionServiceTest {
    ...
    void shouldNotCreateGameQuestionWithBlankCode() {
        when(gameRepository.findGameByCode("")).thenReturn(null);
        when(questionRepository.findById(1)).thenReturn(makeQuestion());
        when(gameQuestionRepository.addGameQuestion("", 1)).thenReturn(true);
    ...
    void shouldNotCreateGameQuestionWithInvalidCode() {
        when(gameRepository.findGameByCode("ZZYX")).thenReturn(null);
        when(questionRepository.findById(1)).thenReturn(makeQuestion());
        when(gameQuestionRepository.addGameQuestion("ZZYX", 1)).thenReturn(true);
    ...
    void shouldNotCreateGameQuestionWithInvalidQuestion() {
        when(gameRepository.findGameByCode("ZZYX")).thenReturn(makeGame());
        when(questionRepository.findById(1)).thenReturn(null);
        when(gameQuestionRepository.addGameQuestion("WXyZ", 1500000)).thenReturn(true);
